...
namespace CppCoverageTest
{
    namespace
    {
        using UnifiedDiffCoverageFilters =
            cov::UnifiedDiffCoverageFilterManager::UnifiedDiffCoverageFilters;

        //-------------------------------------------------------------------------
        UnifiedDiffCoverageFilters CreateFilter(const std::vector<fs::path>& paths,
        ...
        //-------------------------------------------------------------------------
        std::unique_ptr<cov::UnifiedDiffCoverageFilterManager>
        CreateFilterManager(UnifiedDiffCoverageFilters&& filters)
        {
        ...
        //-------------------------------------------------------------------------
        std::unique_ptr<cov::UnifiedDiffCoverageFilterManager>
        CreateFilterManager(const std::vector<fs::path>& paths)
        {
        ...
        size_t GetUnmatchPathsCount(size_t pathCount, size_t maxUnmatchPaths)
        {
            ...
            return filterManager->ComputeWarningMessageLines(maxUnmatchPaths).size() -
                   messageLineCount;
        }
    } // namespace
    ...
    //-------------------------------------------------------------------------
    bool IsLineSelected(int lineNumber, const std::vector<int>& selectedLines)
    {
