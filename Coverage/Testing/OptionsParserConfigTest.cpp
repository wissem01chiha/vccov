...
namespace CppCoverageTest
{
    namespace
    {
            ...
            const std::vector<std::pair<std::string, std::wstring>>& commandLineArguments)
        {
            ...
            for (const auto& argument : configArguments)
                ofs << Tools::LocalToWString(argument.first) << L"=" << argument.second
                    << std::endl;

            std::vector<std::string> arguments{ "--" + cov::ProgramOptions::ConfigFileOption,
                                                path.GetPath().string() };
    ...
    TEST(OptionsParserConfigTest, ConfigurationFileMultipleValue)
    {
        ...
        auto options =
            MutipleSourceParse({ { cov::ProgramOptions::SelectedSourcesOption, source1 },
