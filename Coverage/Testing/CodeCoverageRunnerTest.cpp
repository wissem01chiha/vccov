...
namespace CppCoverageTest
{
    ...
    class CodeCoverageRunnerTest : public ::testing::Test
    {
      public:
        ...
        //---------------------------------------------------------------------
        void TestLine(const Plugin::FileCoverage& file, unsigned int lineNumber,
        ...
        //---------------------------------------------------------------------
        Plugin::CoverageData ComputeCoverageDataInBothMode(const std::wstring& programArg,
        ...
        //---------------------------------------------------------------------
        Plugin::CoverageData RunCoverageWithException(const std::wstring& programArg,
                                                      bool                continueAfterCppException)
        {
            return ComputeCoverageDataInBothMode(
                programArg, TestCoverageConsole::GetOutputBinaryPath().wstring(),
                TestCoverageConsole::GetMainCppFilename().wstring(), continueAfterCppException);
        }

        //---------------------------------------------------------------------
        Plugin::CoverageData ComputeCoverageData(const std::wstring& programArg,
                                                 const std::wstring& fileArgument)
        {
            return ComputeCoverageDataInBothMode(
                programArg, TestCoverageConsole::GetOutputBinaryPath().filename().wstring(),
                fileArgument);
        }
        ...
        intptr_t CountExecutedLines(const Plugin::FileCoverage& file)
        {
            ...
            return boost::count_if(lines, [](const auto& line) { return line.HasBeenExecuted(); });
        }
    ...
    TEST_F(CodeCoverageRunnerTest, RunCoverage)
    {
        ...
        Plugin::CoverageData coverageData =
            ComputeCoverageData(TestCoverageConsole::TestBasic, testBasicFilename);
    ...
    TEST_F(CodeCoverageRunnerTest, RunCoverageDll)
    {
        ...
        Plugin::CoverageData coverageData = ComputeCoverageDataInBothMode(
            TestCoverageConsole::TestSharedLib,
    ...
    TEST_F(CodeCoverageRunnerTest, RunThread)
    {
        ...
        Plugin::CoverageData coverageData =
            ComputeCoverageData(TestCoverageConsole::TestThread, filter);
        auto& file = GetFirstFileCoverage(coverageData);
    ...
    {
        Plugin::CoverageData coverageData =
            RunCoverageWithException(TestCoverageConsole::TestThrowHandledException, false);
    ...
    {
        Plugin::CoverageData coverageData =
            RunCoverageWithException(TestCoverageConsole::TestThrowUnHandledSEHException, false);
    ...
    {
        auto coverageData =
            RunCoverageWithException(TestCoverageConsole::TestThrowUnHandledCppException, false);
        ASSERT_NE(std::string::npos,
                  GetError().find(cov::ExceptionHandler::UnhandledExceptionErrorMessage));
    ...
    {
        auto coverageData =
            RunCoverageWithException(TestCoverageConsole::TestThrowUnHandledCppException, true);
        auto& file       = GetFirstFileCoverage(coverageData);
    ...
    {
        std::vector<std::wstring> arguments = { TestCoverageConsole::TestChildProcess,
    ...
    {
        auto coverageData =
            TestTools::ComputeCoverageData({ TestCoverageConsole::TestChildProcess,
    ...
    {
        const auto specialLineInfoFilename =
            TestCoverageConsole::GetSpecialLineInfoFilename().wstring();

        auto coverageData =
            ComputeCoverageData(TestCoverageConsole::TestSpecialLineInfo, specialLineInfoFilename);
        auto& file = GetFirstFileCoverage(coverageData);
        ASSERT_TRUE(boost::algorithm::iequals(specialLineInfoFilename,
                                              file.GetPath().filename().wstring()));
    ...
    TEST_F(CodeCoverageRunnerTest, UnifiedDiff)
    {
        ...
        CoverageArgs args{ { TestCoverageConsole::TestDiff },
                           TestCoverageConsole::GetOutputBinaryPath().wstring(),
    ...
    TEST_F(CodeCoverageRunnerTest, UnloadReloadDll)
    {
        ...
        auto coverageData = ComputeCoverageDataInBothMode(
            TestCoverageConsole::TestUnloadReloadDll, TestHelper::GetOutputBinaryPath().wstring(),
            unloadDllFilename);
        const auto& file     = GetFirstFileCoverage(coverageData);
        auto        filename = file.GetPath().filename().wstring();
    ...
    TEST_F(CodeCoverageRunnerTest, OptimizedBuild)
    {
        ...
        args.programToRun_ = optimizedBuildProjectPath / "Release" / "OptimizedBuildVS2013.exe";

        if (!Tools::FileExists(args.programToRun_))
        {
            throw std::runtime_error("You need to compile " + optimizedBuildProjectPath.string() +
    ...
    {
        CoverageArgs args{ { TestCoverageConsole::TestBasic },
                           TestCoverageConsole::GetOutputBinaryPath().wstring(),
    ...
    TEST_F(CodeCoverageRunnerTest, SubstitutePdbSourcePath)
    {
        ...
        auto         originalPath         = GetFirstFileCoverage(originalCoverageData).GetPath();

        ASSERT_EQ(filename, originalPath.filename());

        TestHelper::TemporaryPath folder{ TestHelper::TemporaryPathOption::CreateAsFolder };
