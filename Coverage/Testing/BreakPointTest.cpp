...
namespace CppCoverageTest
{
    namespace
    {
        ...
        //---------------------------------------------------------------------
        std::vector<unsigned char> GenerateValues(int valueCount, int moduloValue)
        {
        ...
        //---------------------------------------------------------------------
        std::map<DWORD64, unsigned char>
        BuildOldInstructionsMap(BreakPoint::InstructionCollection& oldInstructionCollection,
        ...
        //---------------------------------------------------------------------
        template <typename T> DWORD64 ToDWORD64(const T* address)
        {
    ...
    TEST(BreakPointTest, SetBreakPoints)
    {
        ...
        auto oldInstructionCollection =
            breakPoint.SetBreakPoints(GetCurrentProcess(), std::move(addresses));
        auto oldInstructionsMap = BuildOldInstructionsMap(oldInstructionCollection, addresses);
