...
namespace CppCoverageTest
{
    namespace
    {
        ...
        CreateOptionParser(std::vector<cov::ExportPluginDescription>&& exportPluginDescription = {})
        {
            std::vector<std::unique_ptr<cov::IOptionParser>> optionParsers;

            optionParsers.push_back(
                std::make_unique<cov::ExportOptionParser>(std::move(exportPluginDescription)));
            return std::make_unique<cov::OptionsParser>(nullptr, std::move(optionParsers));
        }

        //-------------------------------------------------------------------------
        void TestExportTypes(const std::vector<std::string>&        exportTypesStr,
    ...
    {
        TestExportTypes({ cov::ExportOptionParser::ExportTypeCoberturaValue },
    ...
    TEST(OptionsParserExportTest, OutputPath)
    {
        ...
        TestExportTypes({ cov::ExportOptionParser::ExportTypeHtmlValue +
                        ...
                        { cov::OptionsExport{ cov::OptionsExportType::Html, L"",
                                              Tools::LocalToWString(path) } });
    }
    ...
    TEST(OptionsParserExportTest, ExistingExportPath)
    {
        ...
        std::string exportStr = cov::ExportOptionParser::ExportTypeHtmlValue +
                                cov::ExportOptionParser::ExportSeparator +
                                temporaryPath.GetPath().string();
        auto options = TestTools::Parse(
            *parser, { TestTools::GetOptionPrefix() + cov::ExportOptionParser::ExportTypeOption,
                       exportStr });
    ...
    namespace
    {
        ...
        void CheckExportPluginParser(
            const std::wstring& pluginName, const std::wstring& pluginArg,
            ...
            std::function<void(const cov::OptionsExport&)>          checkOptionExporFct)
        {
            ...
            exportPluginDescription.push_back(
                cov::ExportPluginDescription{ std::wstring{ pluginName }, L"", checkArgumentFct });
            auto parser = CreateOptionParser(std::move(exportPluginDescription));

            auto options = TestTools::Parse(
                *parser,
                { TestTools::GetOptionPrefix() + cov::ExportOptionParser::ExportTypeOption,
                  Tools::ToLocalString(pluginName) + ':' + Tools::ToLocalString(pluginArg) });
            if (!options)
    ...
    TEST(OptionsParserExportTest, ExportPluginDescription)
    {
        ...
        CheckExportPluginParser(
            L"pluginName", pluginArg, [&](const auto& arg) { argument = arg; },
