...
namespace CppCoverage
{
        ...
        struct ModuleKind : private Tools::IPEFileHeaderHandler
        {
          ...
          private:
            ...
            void OnNtHeader(const T_IMAGE_NT_HEADERS& ntHeaders)
            {
                ...
                isNativeModule_ = dataDirectory.VirtualAddress == 0 && dataDirectory.Size == 0;
            }

            //-----------------------------------------------------------------
            void OnNtHeader32(HANDLE, DWORD64, const IMAGE_NT_HEADERS32& ntHeader) override
            {
                OnNtHeader(ntHeader);
            }

            //-----------------------------------------------------------------
            void OnNtHeader64(HANDLE, DWORD64, const IMAGE_NT_HEADERS64& ntHeader) override
            {
                OnNtHeader(ntHeader);
            }
    ...
    MonitoredLineRegister::MonitoredLineRegister(
        ...
        : breakPoint_{ breakPoint }, executedAddressManager_{ executedAddressManager },
    ...
    //----------------------------------------------------------------------------
    bool MonitoredLineRegister::RegisterLineToMonitor(const std::filesystem::path& modulePath,
                                                      HANDLE hProcess, void* baseOfImage)
    {
        ...
        moduleInfo_ = std::make_unique<FileFilter::ModuleInfo>(hProcess, modulePath, baseOfImage);

        return debugInformationEnumerator_->Enumerate(modulePath, *this);
    }

    //--------------------------------------------------------------------------
    bool MonitoredLineRegister::IsSourceFileSelected(const std::filesystem::path& path)
    {
    ...
    //--------------------------------------------------------------------------
    void MonitoredLineRegister::OnSourceFile(const std::filesystem::path& path,
                                             const std::vector<Line>&     lines)
    {
        ...
        {
            lineInfos.emplace_back(line.lineNumber_, line.virtualAddress_, line.symbolIndex_);
        }
        ...
        for (const auto& lineInfo : fileInfo.lineInfoColllection_)
        {
            ...
            if (coverageFilterManager_->IsLineSelected(moduleInfo, fileInfo, lineInfo))
            {
                auto addressValue =
                    lineInfo.virtualAddress_ + reinterpret_cast<DWORD64>(moduleInfo.baseOfImage_);

                lineNumberByAddress[addressValue].push_back(lineNumber);
                addresses.push_back(addressValue);
            }
        }
        SetBreakPoint(path, moduleInfo.hProcess_, std::move(addresses), lineNumberByAddress);
    }

    //--------------------------------------------------------------------------
    void MonitoredLineRegister::SetBreakPoint(const std::filesystem::path& path, HANDLE hProcess,
                                              std::vector<DWORD64>&&     addressCollection,
                                              const LineNumberByAddress& lineNumberByAddress)
    {
        auto oldInstructions = breakPoint_->SetBreakPoints(hProcess, std::move(addressCollection));
        for (const auto& value : oldInstructions)
        {
            ...
            {
                Address     address{ hProcess, reinterpret_cast<void*>(addressValue) };
                const auto& lineNumbers = it->second;
                for (auto lineNumber : lineNumbers)
                {
                    if (!executedAddressManager_->RegisterAddress(address, path.wstring(),
                                                                  lineNumber, oldInstruction))
                    {
