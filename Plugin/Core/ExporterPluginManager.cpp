...
{
    //---------------------------------------------------------------------
    std::string InvalidPluginError(const std::optional<std::string>& functionName,
                                   const std::string&                additionalInformation,
                                   const std::filesystem::path&      pluginPath)
    {
        std::string fullMessage = "There was an error when trying to load the export plugin " +
                                  pluginPath.string() + '\n';
    ...
    decltype(auto) CallPluginfunction(PluginFct pluginFct, const std::string& functionName,
                                      const std::filesystem::path& pluginPath, Args&&... args)
    {
        return Tools::Try<std::runtime_error>(
            [&]() { return pluginFct(std::forward<Args>(args)...); },
            [&](const auto& error) { return InvalidPluginError(functionName, error, pluginPath); });
    }
                      ...
                      const std::filesystem::path& pluginPath)
    {
        ...
        auto       pluginVersion = CallPluginfunction(
            [&]() { return exportPlugin.GetExportPluginVersion(); }, functionName, pluginPath);
        auto currentVersion = Plugin::CurrentExportPluginVersion;
        if (pluginVersion != currentVersion)
        {
            auto error = "IExportPlugin version missmatch: "
                         "OpenCppCoverage version is " +
                         std::to_string(currentVersion) + " whereas the plugin version is " +
                         std::to_string(pluginVersion);
            throw std::runtime_error(InvalidPluginError(functionName, error, pluginPath));
        }
    }
    ...
    {
        for (auto pluginPath : std::filesystem::directory_iterator(pluginFolder_))
        {
            ...
            auto plugin = Tools::Try<std::runtime_error>(
                [&]() { return pluginLoader.TryLoadPlugin(pluginPath, pluginFactoryFctName); },
                [&](const auto& error) { return InvalidPluginError(std::nullopt, error, path); });
    ...
    //-------------------------------------------------------------------------
    void CheckArgument(const std::weak_ptr<LoadedPlugin<Plugin::IExportPlugin>>& weakPlugin,
                       const std::filesystem::path&                              pluginPath,
    ...
    {
        std::vector<CppCoverage::ExportPluginDescription> exportPluginDescriptions;
        ...
        for (const auto& plugin : plugins_)
        {
            ...
            auto helpDescription = CallPluginfunction(
                ...
                "GetHelpDescription", pluginPath);

            std::weak_ptr<LoadedPlugin<Plugin::IExportPlugin>> weakPlugin{ exportPlugin };

            exportPluginDescriptions.push_back(CppCoverage::ExportPluginDescription{
                std::wstring{ pluginName }, std::move(helpDescription),
                [weakPlugin, pluginPath, this](const std::optional<std::wstring>& parameter)
    ...
    //-------------------------------------------------------------------------
    void ExporterPluginManager::Export(const std::wstring&                pluginName,
                                       ...
                                       const std::optional<std::wstring>& argument) const
    {
        ...
        auto optionalOutput =
            CallPluginfunction([&]() { return plugin->Get().Export(coverageData, argument); },
                               "Export", pluginFolder_ / pluginName);
        if (optionalOutput)
        {
            Tools::ShowOutputMessage(pluginName + L" has generated the report at ",
                                     *optionalOutput);
        }
    }
} // namespace Exporter
