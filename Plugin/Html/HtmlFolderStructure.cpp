...
namespace Exporter
{
    ...
    //-------------------------------------------------------------------------
    std::filesystem::path
    HtmlFolderStructure::CreateCurrentRoot(const std::filesystem::path& outputFolder)
    {
        auto root{ fs::absolute(outputFolder) };
        optionalCurrentRoot_ = std::make_unique<Hierarchy>(root);
        CopyRecursiveDirectoryContent(templateFolder_ / HtmlFolderStructure::ThirdParty,
    ...
    HtmlFile HtmlFolderStructure::GetHtmlFilePath(const std::filesystem::path& filePath) const
    {
        ...
        auto        fileHtmlPath =
            optionalCurrentModule_->uniqueChildrenPath_.GetUniquePath(modulePath / output);
