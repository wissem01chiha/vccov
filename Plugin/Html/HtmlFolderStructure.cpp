...
struct HtmlFolderStructure::Hierarchy {
  explicit Hierarchy(const std::filesystem::path &path) : path_{path} {
...
//-------------------------------------------------------------------------
HtmlFolderStructure::HtmlFolderStructure(
...
//-------------------------------------------------------------------------
std::filesystem::path HtmlFolderStructure::CreateCurrentRoot(
    const std::filesystem::path &outputFolder) {
  auto root{fs::absolute(outputFolder)};
  optionalCurrentRoot_ = std::make_unique<Hierarchy>(root);
  CopyRecursiveDirectoryContent(templateFolder_ /
                                    HtmlFolderStructure::ThirdParty,
...
//-------------------------------------------------------------------------
HtmlFile HtmlFolderStructure::CreateCurrentModule(
  ...
  auto folderModules =
      optionalCurrentRoot_->path_ / HtmlFolderStructure::FolderModules;

  auto moduleFolder = folderModules / moduleName;
  auto uniqueModulesFolder =
      optionalCurrentRoot_->uniqueChildrenPath_.GetUniquePath(moduleFolder);
  ...
  return HtmlFile{moduleHtmlPath, fs::path{HtmlFolderStructure::FolderModules} /
                                      moduleHtmlPath.filename()};
}

//---------------------------------------------------------------------
HtmlFile HtmlFolderStructure::GetHtmlFilePath(
  ...
  auto fileHtmlPath = optionalCurrentModule_->uniqueChildrenPath_.GetUniquePath(
      modulePath / output);

  return HtmlFile{fileHtmlPath,
                  modulePath.filename() / fileHtmlPath.filename()};
}
} // namespace Exporter
