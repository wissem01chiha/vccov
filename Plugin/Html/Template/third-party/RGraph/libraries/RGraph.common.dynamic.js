...
/**
 * This is the window click event listener. It redraws all canvas tags on the
 * page.
 */
RGraph.installWindowMousedownListener = RGraph.InstallWindowMousedownListener = function(obj) {
 ...
 * page.
 */
RGraph.installWindowMouseupListener = RGraph.InstallWindowMouseupListener = function(obj) {
...
/**
 * This is the canvas mouseup event listener. It installs the mouseup event for
 * the canvas. The mouseup event then checks the relevant object.
 *
 * @param object obj The chart object
 *
 * RGraph.window_mouseup_event_listener
 */
RGraph.installCanvasMouseupListener = RGraph.InstallCanvasMouseupListener = function(obj) {
    if (!obj.canvas.rgraph_mouseup_event_listener)
    {
        obj.canvas.rgraph_mouseup_event_listener = function(e) {
            ...
            // This causes things at the edge of the chart area - eg line chart
            // hotspots - not to fire because the cursor is out of the chart area
            var objects = RGraph.ObjectRegistry.getObjectsByXY(e);
            // var objects =
            // RGraph.ObjectRegistry.getObjectsByCanvasID(e.target.id);
            ...
            if (objects)
            {
                for (var i = 0, len = objects.length; i < len; i += 1)
                {
                    ...
                    if (RGraph.Registry.Get('chart.adjusting') ||
                        RGraph.Registry.Get('chart.adjusting.gantt'))
                    {
                        RGraph.FireCustomEvent(RGraph.Registry.Get('chart.adjusting'),
                                               'onadjustend');
                    }
                    ...
                    /**
                     * If the mouse pointer is over a "front" chart this prevents
                     * charts behind it from firing their events.
                     */
 ...
 * @param object obj The chart object
 */
RGraph.installCanvasMousemoveListener = RGraph.InstallCanvasMousemoveListener = function(obj) {
    if (!obj.canvas.rgraph_mousemove_event_listener)
    {
        obj.canvas.rgraph_mousemove_event_listener = function(e) {
            ...
            /**
             * Go through all the objects and check them to see if anything needs
             * doing
             */
            var objects = RGraph.ObjectRegistry.getObjectsByXY(e);
            ...
            if (objects && objects.length > 0)
            {
                for (var i = 0, len = objects.length; i < len; i += 1)
                {
                    ...
                    if (shape)
                    {
                        var index = shape['object'].type == 'scatter' ? shape['index_adjusted']
                                                                      : shape['index'];
                        if (typeof (obj['$' + index]) == 'object' &&
                            typeof (obj['$' + index].onmousemove) == 'function')
                        {
                    ...
                    /**
                     * This bit saves the current pointer style if there isn't one already
                     * saved
                     */
                    if (shape && (typeof (func) == 'function' || typeof (func2) == 'function' ||
                                  typeof obj.Get('link') === 'string'))
                    {
                    ...
                    /**
                     * This facilitates breaking out of the loop when a shape has been
                     * found - ie the cursor is over a shape an upper chart
                     */
                    if (shape || (obj.overChartArea && obj.overChartArea(e)))
                    {
                        break;
            ...
            // InteractiveKey_line_mousemove(e); if (typeof
            // InteractiveKey_pie_mousemove == 'function')
 ...
 * @param object obj The chart object
 */
RGraph.installCanvasMousedownListener = RGraph.InstallCanvasMousedownListener = function(obj) {
            ...
            if (obj)
            {
                ...
                if (obj && obj.isRGraph && obj.Get('chart.adjustable'))
                {
                    ...
                    if (obj && obj.isRGraph)
                    {
                        ...
                        // If applicable, get the appropriate shape and store it in the
                        // registry
                        switch (obj.type)
                        {
                        case 'bar':
                        ...
                        case 'gantt':
                            ...
                            if (shape)
                            {
                                ...
                                RGraph.Registry.Set('chart.adjusting.gantt', {
                                    ...
                                    'mode' :
                                        (mouseXY[0] > (shape['x'] + shape['width'] - 5) ? 'resize'
                                                                                        : 'move'),
                                    'shape' : shape
                                });
                            }
                        ...
                        // Call the mousemove event listener so that the canvas is
                        // adjusted even though the mouse isn't moved
 ...
 * @param object obj The chart object
 */
RGraph.installCanvasClickListener = RGraph.InstallCanvasClickListener = function(obj) {
    if (!obj.canvas.rgraph_click_event_listener)
    {
        obj.canvas.rgraph_click_event_listener = function(e) {
            ...
            for (var i = 0, len = objects.length; i < len; i += 1)
            {
                ...
                /**
                 * This bit saves the current pointer style if there isn't one
                 * already saved
                 */
...
/**
 * This function evaluates the various cursor settings and if there's one for
 * pointer, changes it to that
 */
// RGraph.evaluateCursor =
RGraph.evaluateCursor = RGraph.EvaluateCursor = function(e) {
    var obj     = null;
    var mouseXY = RGraph.getMouseXY(e);
    ...
    for (var i = 0, len = objects.length; i < len; i += 1)
    {
        if ((objects[i].getShape && objects[i].getShape(e)) ||
            (objects[i].overChartArea && objects[i].overChartArea(e)))
        {
            var obj = objects[i];
            var id  = obj.id;
        }
    ...
    if (!RGraph.is_null(obj))
    {
        if (obj.getShape && obj.getShape(e))
        {
            ...
            if (obj.Get('chart.tooltips'))
            {
                ...
                {
                    text =
                        RGraph.parseTooltipText(obj.Get('chart.tooltips'), shape['index_adjusted']);
                }
        ...
        if (!RGraph.is_null(obj) && obj.Get('chart.key.interactive'))
        {
            for (var j = 0; j < obj.coords.key.length; ++j)
            {
                if (mouseX > obj.coords.key[j][0] &&
                    mouseX < (obj.coords.key[j][0] + obj.coords.key[j][2]) &&
                    mouseY > obj.coords.key[j][1] &&
                    mouseY < (obj.coords.key[j][1] + obj.coords.key[j][3]))
                {
                    var pointer = true;
                }
            }
    ...
    /**
     * It can be specified in the user mousemove event - remember it can now be
     * specified in THREE ways
     */
    if (!RGraph.is_null(shape) && !RGraph.is_null(obj))
    {

        if (!RGraph.is_null(obj.Get('chart.events.mousemove')) &&
 ...
 *
 * @param mixed tooltip This could be a string or a function. If it's a function
 *     it's called and
 ...
 *
 * @param object obj The graph object (from which we can get the context and
 *     canvas as required)
 */
RGraph.drawCrosshairs = RGraph.DrawCrosshairs = function(e, obj) {
    ...
    if (x >= gutterLeft && y >= gutterTop && x <= (width - gutterRight) &&
        y <= (height - gutterBottom))
    {
        ...
        if (prop['chart.crosshairs.snap'])
        {
            ...
            if (obj.type == 'line')
            {
                ...
                for (var dataset = 0; dataset < obj.coords2.length; ++dataset)
                {
                    ...
                    {
                        if (obj.coords2[dataset][point][0] == x &&
                            obj.coords2[dataset][point][1] == y)
                        {
            ...
            else
            {
                ...
                for (var i = 0; i < obj.coords.length; ++i)
                {
                    for (var j = 0; j < obj.coords[i].length; ++j)
                    {
                        ...
                        // Check the mouse X coordinate
                        var len =
                            RGraph.getHypLength(obj.coords[i][j][0], obj.coords[i][j][1], x, y);
        ...
        if (obj.type == 'scatter' && prop['chart.crosshairs.coords'])
        {
            ...
            var xCoord = (((x - gutterLeft) / (width - gutterLeft - gutterRight)) *
                          (prop['chart.xmax'] - prop['chart.xmin'])) +
                         prop['chart.xmin'];
            xCoord = xCoord.toFixed(prop['chart.scale.decimals']);
            var yCoord =
                obj.max -
                (((y - prop['chart.gutter.top']) / (height - gutterTop - gutterBottom)) * obj.max);
            ...
            div.innerHTML = '<span style="color: #666">' +
                            prop['chart.crosshairs.coords.labels.x'] + ':</span> ' + xCoord +
                            '<br><span style="color: #666">' +
                            prop['chart.crosshairs.coords.labels.y'] + ':</span> ' + yCoord;
