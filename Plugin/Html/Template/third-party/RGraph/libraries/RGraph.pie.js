...
RGraph.Pie =
    ...
    this.properties = {
        'chart.colors' : [
            'Gradient(red:#fcc)', 'Gradient(#ddd:#eee)', 'Gradient(#0f0:#cfc)',
            'Gradient(blue:#ccf)', 'Gradient(#FB7BA3:#FCC7EE)', 'Gradient(yellow:#ffc)',
            'Gradient(#000:#ccc)', 'Gradient(#EE9D80:#FEE5C8)', 'Gradient(cyan:#ccf)',
            'Gradient(#9E7BF6:#C7B6D2)', 'Gradient(#78CAEA:#C5FBFD)', 'Gradient(#E284E9:#FDC4FF)',
            'Gradient(white:#ccf)', 'Gradient(blue:#ccf)', 'Gradient(#9E7BF6:#C7B6D2)'
        ],
    ...
    /**
     * Translate half a pixel for antialiasing purposes - but only if it hasn't
     * beeen done already
     */
    ...
    /**
     * "Decorate" the object with the generic effects if the effects library has
     * been included
     */
    ...
    this.set = this.Set = function(name, value) {
        ...
        /**
         * This should be done first - prepend the property name with "chart." if
         * necessary
         */
    ...
    this.draw = this.Draw = function() {
        ...
        this.centerx = (this.graph.width / 2) + this.gutterLeft this.centery =
                           (this.graph.height / 2) + this.gutterTop this.subTotal =
                               this.properties['chart.origin'];
        this.angles     = [];
        this.coordsText = [];
        ...
        /**
         * Parse the colors for gradients. Its down here so that the center X/Y can
         * be used
         */
        ...
        /**
         * This sets the label colors. Doing it here saves lots of if() conditions
         * in the draw method
         */
        if (prop['chart.labels.colors'].length < prop['chart.labels'].length)
        {
            ...
            {
                prop['chart.labels.colors'].push(
                    prop['chart.labels.colors'][prop['chart.labels.colors'].length - 1]);
            }
        }
        else
        {
         ...
         * Draw the title
         */
        RG.DrawTitle(this, prop['chart.title'], (ca.height / 2) - this.radius - 5, this.centerx,
        ...
        //
        //     co.arc(this.centerx + offsetx, this.centery + offsety, this.radius,
        //     0, TWOPI, 0);
        //
        //     co.fill();
        //
        ...
        /**
         * Now draw the segments again with shadow turned off. This is always
         * performed, not just if the shadow is on.
         */
        ...
        for (var action = 0; action < 2; action += 1)
        {
            for (var i = 0; i < len; i++)
            {
                ...
                if (action === 1)
                {
                    co.strokeStyle = typeof (prop['chart.strokestyle']) == 'object'
                                         ? prop['chart.strokestyle'][i]
                ...
                if (prop['chart.variant'] == 'donut')
                {
                    ...
                    co.arc(segment[2], segment[3],
                           typeof (prop['chart.variant.donut.width']) == 'number'
                               ? r - prop['chart.variant.donut.width']
                               : r / 2,
                           (segment[1]), (segment[0]), true);
                }
                else
                {
        ...
        if (prop['chart.labels.sticks'])
        {
            ...
            // Redraw the border going around the Pie chart if the stroke style is NOT
            // white
    ...
    this.drawSegment = this.DrawSegment = function(radians, color, last, index) {
        // IE7/8/ExCanvas fix (when there's only one segment the Pie chart doesn't
        // display
        ...
        {
            RG.SetShadow(this, prop['chart.shadow.color'], prop['chart.shadow.offsetx'],
                         prop['chart.shadow.offsety'], prop['chart.shadow.blur']);
        }

        /**
         * Exploded segments
         */
        if ((typeof (prop['chart.exploded']) == 'object' && prop['chart.exploded'][index] > 0) ||
            typeof (prop['chart.exploded']) == 'number')
        {

            var explosion = typeof (prop['chart.exploded']) == 'number'
                                ? prop['chart.exploded']
                                : prop['chart.exploded'][index];
            var x         = 0;
            var y         = 0;
            var h         = explosion;
        ...
        if (prop['chart.variant'] == 'donut')
        {
            ...
            co.arc(this.centerx + x, this.centery + y,
                   typeof (prop['chart.variant.donut.width']) == 'number'
                       ? r - prop['chart.variant.donut.width']
                       : r / 2,
    ...
    this.drawLabels = this.DrawLabels = function() {
        ...
        if (labels && labels.length)
        {
            ...
            for (i = 0; i < this.angles.length; ++i)
            {
                 ...
                 */
                if (typeof prop['chart.exploded'] === 'object' && prop['chart.exploded'][i] ||
                    typeof (prop['chart.exploded']) == 'number')
                {

                    var t          = ((segment[1] - segment[0]) / 2);
                    var seperation = typeof (prop['chart.exploded']) == 'number'
                                         ? prop['chart.exploded']
                                         : prop['chart.exploded'][i];
                 ...
                 * Coords for the text
                 */
                var x = cx + explosion_offsetx + ((r + 10) * Math.cos(a)) +
                        (prop['chart.labels.sticks']
                             ? (a < RG.HALFPI || a > (RG.TWOPI + RG.HALFPI) ? 2 : -2)
                             : 0)
                ...
                if (typeof (prop['chart.labels.colors']) == 'object' &&
                    prop['chart.labels.colors'] && prop['chart.labels.colors'][i])
                {
    ...
    this.drawSticks = this.DrawSticks = function() {
        ...
        for (var i = 0, len = this.angles.length; i < len; ++i)
        {
            ...
            points[1] = RG.getRadiusEndPoint(
                cx, cy, midpoint, radius + prop['chart.labels.sticks.length'] + extra - 5);

            points[2] = RG.getRadiusEndPoint(cx, cy, midpoint,
                                             radius + prop['chart.labels.sticks.length'] + extra);

            points[3] = RG.getRadiusEndPoint(cx, cy, midpoint,
                                             radius + prop['chart.labels.sticks.length'] + extra);
            points[3][0] += (points[3][0] > cx ? 5 : -5);

            points[4] = [
                points[2][0] + (points[2][0] > cx ? 5 + prop['chart.labels.sticks.hlength']
                                                  : -5 - prop['chart.labels.sticks.hlength']),
                points[2][1]
            ];
    ...
    this.getShape = this.getSegment = function(e) {
        ...
        for (var i = 0, len = angles.length; i < len; ++i)
        {
            ...
            {
                co.arc(angles[i][2], angles[i][3],
                       (typeof (prop['chart.variant.donut.width']) == 'number'
                            ? this.radius - prop['chart.variant.donut.width']
                            : this.radius / 2),
                       angles[i][1], angles[i][0], true);
            }
            else
            {
             ...
             */
            var tooltip =
                RG.parseTooltipText ? RG.parseTooltipText(prop['chart.tooltips'], ret[5]) : null;
    ...
    this.explodeSegment = this.Explode = function(index, size) {
        ...
        prop['chart.exploded'][index] =
            typeof (original_explode) == 'number' ? original_explode : 0;
     ...
     *
     * @param array segment The segment information that is returned by the
     *     pie.getSegment(e) function
     */
    this.highlight_segment = function(segment) {
        ...
        co.arc(segment[0], segment[1], segment[2], this.angles[segment[5]][0],
               this.angles[segment[5]][1], 0);
    ...
    /**
     * Each object type has its own Highlight() function which highlights the
     * appropriate shape
     *
    ...
    this.highlight = this.Highlight = function(shape) {
        if (prop['chart.tooltips.highlight'])
        {
            ...
            if (prop['chart.highlight.style'] == '3d')
            {
                ...
                // This is the extent of the 2D effect. Bigger values will give the
                // appearance of a larger "protusion"
                var extent = 2;
                ...
                co.arc(shape['x'], shape['y'], shape['radius'], shape['angle.start'],
                       shape['angle.end'], false);
                if (prop['chart.variant'] == 'donut')
                {
                    co.arc(shape['x'], shape['y'], shape['radius'] / 5, shape['angle.end'],
                           shape['angle.start'], true);
                }
                else
                {
                ...
                co.strokeStyle = prop['chart.strokestyle'];
                co.arc(shape['x'] - extent, shape['y'] - extent, shape['radius'],
                       shape['angle.start'], shape['angle.end'], false);
                if (prop['chart.variant'] == 'donut')
                {
                    co.arc(shape['x'] - extent, shape['y'] - extent, shape['radius'] / 2,
                           shape['angle.end'], shape['angle.start'], true)
                }
                else
                {
            ...
            else
            {
                ...
                {
                    co.arc(shape['x'], shape['y'], shape['radius'] + 1, shape['angle.start'],
                           shape['angle.end'], false);
                    co.lineTo(shape['x'], shape['y']);
                }
                co.closePath();
    ...
    this.drawCenterpin = this.DrawCenterpin = function() {
        if (typeof (prop['chart.centerpin']) == 'number' && prop['chart.centerpin'] > 0)
        {
            ...
            co.beginPath();
            co.strokeStyle = prop['chart.centerpin.stroke'] ? prop['chart.centerpin.stroke']
                                                            : prop['chart.strokestyle'];
            co.fillStyle   = prop['chart.centerpin.fill'] ? prop['chart.centerpin.fill']
                                                          : prop['chart.strokestyle'];
            co.moveTo(cx, cy);
    ...
    this.positionTooltip = function(obj, x, y, tooltip, idx) {
        ...
        var x           = canvasXY[0] + this.angles[idx][2] +
                (Math.cos(angleCenter) *
                 (prop['chart.variant'] == 'donut' &&
                          typeof (prop['chart.variant.donut.width']) == 'number'
                      ? ((this.radius - prop['chart.variant.donut.width']) +
                         (prop['chart.variant.donut.width'] / 2))
                      : (this.radius * (prop['chart.variant'] == 'donut' ? 0.75 : 0.5))));
        var y = canvasXY[1] + this.angles[idx][3] +
    ...
    this.drawInGraphLabels = this.DrawInGraphLabels = function() {
        ...
        if (prop['chart.variant'] == 'donut')
        {
            ...
            {
                var r = (this.radius - prop['chart.variant.donut.width']) +
                        (prop['chart.variant.donut.width'] / 2);
            }
        }
        else
        ...
        for (var i = 0, len = this.angles.length; i < len; ++i)
        {
            ...
            if (typeof (prop['chart.exploded']) == 'object' &&
                typeof (prop['chart.exploded'][i]) == 'number')
            {
            ...
            var angleCenter = ((angleEnd - angleStart) / 2) + angleStart;
            var coords      = RG.getRadiusEndPoint(this.centerx, this.centery, angleCenter,
                                                   r + (explosion ? explosion : 0));
            var x           = coords[0];
            var y           = coords[1];

            var text =
                prop['chart.labels.ingraph.specific'] &&
                        typeof (prop['chart.labels.ingraph.specific'][i]) == 'string'
                    ? prop['chart.labels.ingraph.specific'][i]
                    : RG.number_format(this, this.data[i], prop['chart.labels.ingraph.units.pre'],
            ...
            if (text)
            {
                ...
                var font = typeof (prop['chart.labels.ingraph.font']) == 'string'
                               ? prop['chart.labels.ingraph.font']
                               : prop['chart.text.font'];
                var size = typeof (prop['chart.labels.ingraph.size']) == 'number'
    ...
    this.parseColors = function() {
        // Save the original colors so that they can be restored when the canvas is
        // reset
        if (this.original_colors.length === 0)
        {
            ...
            this.original_colors['chart.highlight.stroke'] =
                RG.array_clone(prop['chart.highlight.stroke']);
            this.original_colors['chart.highlight.style.2d.fill'] =
                RG.array_clone(prop['chart.highlight.style.2d.fill']);
            this.original_colors['chart.highlight.style.stroke'] =
                RG.array_clone(prop['chart.highlight.style.2d.stroke']);
        }
        ...
        prop['chart.highlight.stroke'] =
            this.parseSingleColorForGradient(prop['chart.highlight.stroke']);
        prop['chart.highlight.style.2d.fill'] =
            this.parseSingleColorForGradient(prop['chart.highlight.style.2d.fill']);
        prop['chart.highlight.style.2d.stroke'] =
            this.parseSingleColorForGradient(prop['chart.highlight.style.2d.stroke']);
    };
    ...
    this.parseSingleColorForGradient = function(color) {
        ...
        if (color.match(/^gradient\((.*)\)$/i))
        {
            ...
            if (prop['chart.variant'] == 'donut')
            {
                var radius_start = typeof (prop['chart.variant.donut.width']) == 'number'
                                       ? this.radius - prop['chart.variant.donut.width']
                                       : this.radius / 2;
            }
            else
            ...
            var grad = co.createRadialGradient(
                this.centerx, this.centery, radius_start, this.centerx, this.centery,
    ...
    /**
     * This function handles highlighting an entire data-series for the
     * interactive key
     *
     ...
     * Pie chart explode
     *
     * Explodes the Pie chart - gradually incrementing the size of the
     * chart.explode property
     *
     * @param object     Options for the effect
     * @param function   An optional callback function to call when the animation
     *     completes
     */
    this.explode = function() {
        ...
        var frame    = 0;
        var maxExplode =
            Number(typeof opt.radius === 'number' ? opt.radius : ma.max(ca.width, ca.height));
    ...
    /**
     * Now need to register all chart types. MUST be after the setters/getters are
     * defined
     *
