...
namespace FileFilter
{
            ...
            for (auto& pathData : it->second)
            {
                ...
                if (boost::algorithm::ends_with(normalizedPath.wstring(), postFixPath.wstring()))
                {
                    if (pathData.matchedPath_ &&
                        !boost::algorithm::equals(pathData.matchedPath_->wstring(),
                                                  normalizedPath.wstring()))
                    {
                        throw AmbiguousPathException(postFixPath, *pathData.matchedPath_,
                                                     normalizedPath);
                    }
      ...
      private:
        struct PathData
        {
            explicit PathData(File&& fullPath)
                : file_{ std::move(fullPath) }, haveBeenMarched_{ false }
            {
            }
    ...
    //-------------------------------------------------------------------------
    PathMatcher::PathMatcher(std::vector<File>&& files, const boost::optional<fs::path>& parentPath)
    {
        if (parentPath)
            pathMatcherEngine_ =
                std::make_unique<FullPathMatcherEngine>(*parentPath, std::move(files));
        else
