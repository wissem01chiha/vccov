...
namespace FileFilter
{
    ...
    {
        //-------------------------------------------------------------------------
        DWORD64 ExtractRelocations(HANDLE hProcess, DWORD64 baseOfImage,
                                   DWORD64 imageBaseRelocationPtr, int sizeOfPointer,
                                   std::unordered_set<DWORD64>& relocations)
        {
            ...
            Tools::ReadProcessMemory(hProcess,
                                     imageBaseRelocationPtr + sizeof(IMAGE_BASE_RELOCATION),
                                     &relocationPtrs[0], relocationPtrs.size() * sizeof(WORD));
            ...
            for (auto relocationPtr : relocationPtrs)
            {
                ...
                if (relocationType == IMAGE_REL_BASED_HIGHLOW ||
                    relocationType == IMAGE_REL_BASED_DIR64)
                {
                    auto rva = relocationPtr & 0x0fff;
                    auto relocationAddress =
                        imageBaseRelocation->VirtualAddress + rva + baseOfImage;
                    DWORD_PTR relocationValue = 0;
                    Tools::ReadProcessMemory(hProcess, relocationAddress, &relocationValue,
                                             sizeOfPointer);
        ...
        {
            //-----------------------------------------------------------------
            void OnNtHeader32(HANDLE hProcess, DWORD64 baseOfImage,
                              const IMAGE_NT_HEADERS32& ntHeader) override
            {
                FillRelocations(hProcess, baseOfImage,
                                GetRelocationsDirectory(ntHeader, sizeof(DWORD)));
            }

            //-------------------------------------------------------------------------
            void OnNtHeader64(HANDLE hProcess, DWORD64 baseOfImage,
            ...
            //-----------------------------------------------------------------
            void FillRelocations(HANDLE hProcess, DWORD64 baseOfImage,
                                 std::unique_ptr<RelocationsDirectoryInfo> relocationsInfo)
            {
                ...
                while (imageBaseRelocationPtr < endBaseRelocationPtr)
                {
                    imageBaseRelocationPtr +=
                        ExtractRelocations(hProcess, baseOfImage, imageBaseRelocationPtr,
                                           relocationsInfo->sizeOfPointer, relocations_);
                }
            }
    ...
    //-------------------------------------------------------------------------
    std::unordered_set<DWORD64> RelocationsExtractor::Extract(HANDLE  hProcess,
