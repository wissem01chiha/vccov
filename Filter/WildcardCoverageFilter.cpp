...
namespace CppCoverage
{
    ...
    {
        //---------------------------------------------------------------------
        const Wildcards* MatchAny(const std::wstring&           str,
        ...
        //---------------------------------------------------------------------
        std::vector<Wildcards> BuildWildcards(const std::vector<std::wstring>& regexesStr,
    ...
    WildcardCoverageFilter::BuildFilter(const Patterns& patterns) const
    {
        ...
        filter->selectedWildcards =
            BuildWildcards(patterns.GetSelectedPatterns(), patterns.IsRegexCaseSensitiv());
        filter->excludedWildcards =
            BuildWildcards(patterns.GetExcludedPatterns(), patterns.IsRegexCaseSensitiv());

        return filter;
    }

    //---------------------------------------------------------------------
    bool WildcardCoverageFilter::Match(const std::wstring& str, const Filter& filter,
                                       std::wostream& ostr) const
    {
        ...
        if (excludedRegEx)
        {
            ostr << L": " << str << L" is not selected because it matches excluded pattern: "
                 << *excludedRegEx;
            return false;
        }

        ostr << L": " << str << L" is selected because it matches selected pattern: "
             << *selectedRegEx;
        ;
        return true;
    }
