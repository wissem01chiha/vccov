...
namespace FileFilterTest
{
    ...
    fs::path GetDumpBinPath()
    {
        ...
        const auto it =
            std::find_if(fs::recursive_directory_iterator(msvc), end,
    ...
    std::unordered_set<DWORD64> ExtractRelocations(const fs::path& dumpBinPath)
    {
        std::vector<std::string> args = {
            "/RELOCATIONS", TestCoverageOptimizedBuild::GetOutputBinaryPath().string()
        };
    ...
    {
        std::vector<std::string> args = {
            "/HEADERS", TestCoverageOptimizedBuild::GetOutputBinaryPath().string()
        };
        const std::string value = TestHelper::RunProcess(dumpBinPath, args);
        std::smatch       match;
