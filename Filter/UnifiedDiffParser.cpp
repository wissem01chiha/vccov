...
namespace FileFilter
{
    namespace
    {
        ...
        //-----------------------------------------------------------------------
        bool IsGitDetected(const std::vector<File>&         files,
                           const std::vector<std::wstring>& sourceFileLines, bool foundGitHeader)
        {
            if (!foundGitHeader)
                return false;

            auto isGitTarget = std::all_of(files.begin(), files.end(),
                                           [](const auto& file) {
                                               return boost::algorithm::starts_with(
                                                   file.GetPath().wstring(), GitTargetPrefix);
                                           });

            auto isGitSource = std::all_of(sourceFileLines.begin(), sourceFileLines.end(),
                                           [](const auto& line) {
                                               return boost::algorithm::starts_with(
                                                   line, UnifiedDiffParser::FromFilePrefix + L"a/");
                                           });
        ...
        //-----------------------------------------------------------------------
        void UpdateFilePathIfGitDetected(std::vector<File>&               files,
        ...
        //---------------------------------------------------------------------
        template <typename Container, typename Fct> void EraseIf(Container& container, Fct fct)
        {
        ...
        //---------------------------------------------------------------------
        void RemoveDevNull(std::vector<File>& files, std::vector<std::wstring>& sourceFileLines)
        {
            EraseIf(files, [](const auto& file) { return file.GetPath().wstring() == DevNull; });

            EraseIf(sourceFileLines,
                    [](const auto& line) {
                        return boost::algorithm::starts_with(
                            line, UnifiedDiffParser::FromFilePrefix + DevNull);
                    });
        }
    } // namespace
    //---------------------------------------------------------------------
    ...
    {
        Stream(std::wistream& istr) : istr_{ istr }, currentLine_{ 0 }, lastLineRead_{ 0 }
        {
        }
    ...
    //---------------------------------------------------------------------
    void UnifiedDiffParser::FillUpdatedLines(const std::wstring& line, std::vector<File>& files,
                                             Stream& stream) const
    {
        if (files.empty())
    ...
    UnifiedDiffParser::ExtractHunksDifferences(const Stream&       stream,
    ...
    //-------------------------------------------------------------------------
    std::vector<int>
    UnifiedDiffParser::ExtractUpdatedLines(Stream&             stream,
                                           const std::wstring& hunksDifferencesLine) const
    {
