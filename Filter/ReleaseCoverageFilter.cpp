...
namespace FileFilter
{
    ...
    //-------------------------------------------------------------------------
    bool ReleaseCoverageFilter::IsLineSelected(const ModuleInfo& moduleInfo,
                                               const FileInfo& fileInfo, const LineInfo& lineInfo)
    {
        ...
        LOG_DEBUG << "Optimized build support ignores line " << lineInfo.lineNumber_ << " of "
                  << fileInfo.filePath_.wstring();
        return false;
    }

    //-------------------------------------------------------------------------
    void ReleaseCoverageFilter::UpdateCachesIfExpired(const ModuleInfo& moduleInfo,
                                                      const FileInfo&   fileInfo)
    {
        ...
        if (!mModuleData_ || mModuleData_->path_ != modulePath)
        {
            ...
            mModuleData_->relocations_ = relocationsExtractor_->Extract(
                moduleInfo.hProcess_, reinterpret_cast<DWORD64>(moduleInfo.baseOfImage_));
        }
    ...
    ReleaseCoverageFilter::UpdateLineDataCaches(const std::filesystem::path& filePath,
                                                const std::vector<LineInfo>& lineDatas)
    {
