...
namespace CppCoverage
{
        ...
        for (const auto& unifiedDiffSettings : unifiedDiffSettingsCollection)
        {
            unifiedDiffCoverageFilters.emplace_back(
                std::make_unique<FileFilter::UnifiedDiffCoverageFilter>(
                    unifiedDiffSettings.GetUnifiedDiffPath(),
                    unifiedDiffSettings.GetRootDiffFolder()));
        }
    ...
    //-------------------------------------------------------------------------
    boost::optional<int> GetExecutableLineOrPreviousOne(int                  lineNumber,
    ...
    //-------------------------------------------------------------------------
    template <typename Container> std::wstring ToWString(const Container& values)
    {
    ...
    UnifiedDiffCoverageFilterManager::UnifiedDiffCoverageFilterManager(
        const std::vector<UnifiedDiffSettings>& unifiedDiffSettingsCollection)
        : UnifiedDiffCoverageFilterManager{ ToUnifiedDiffCoverageFilters(
    ...
    //-------------------------------------------------------------------------
    bool UnifiedDiffCoverageFilterManager::IsLineSelected(const FileFilter::FileInfo& fileInfo,
                                                          const FileFilter::LineInfo& lineInfo)
    {
        ...
        auto executableLineNumber =
            GetExecutableLineOrPreviousOne(lineInfo.lineNumber_, executableLinesSet);
        if (!executableLineNumber)
            return false;

        return AnyOfOrTrueIfEmpty(
            unifiedDiffCoverageFilters_, [&](const auto& filter)
            { return filter->IsLineSelected(fileInfo.filePath_, *executableLineNumber); });
    }

    //-------------------------------------------------------------------------
    std::vector<std::wstring>
    UnifiedDiffCoverageFilterManager::ComputeWarningMessageLines(size_t maxUnmatchPaths) const
    {
    ...
    std::vector<std::wstring> UnifiedDiffCoverageFilterManager::ComputeWarningMessageLines(
        const std::set<std::filesystem::path>& unmatchPaths, size_t maxUnmatchPaths) const
    {
    ...
    //-------------------------------------------------------------------------
    const std::set<int>&
    UnifiedDiffCoverageFilterManager::GetExecutableLinesSet(const FileFilter::FileInfo& fileInfo)
    {
