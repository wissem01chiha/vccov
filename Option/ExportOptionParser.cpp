...
namespace CppCoverage
{
    namespace
    {
        ...
        //-------------------------------------------------------------------------
        boost::optional<OptionsExport>
        CreateExport(const std::map<std::wstring, OptionsExportType>& exportTypes,
                     const ExportData&                                exportData)
        {
            ...
            if (it != exportTypes.end())
            {
                ...
                return OptionsExport{ type, std::wstring{ exportData.exportType },
        ...
        //-------------------------------------------------------------------------
        bool
        TryAddExportPlugin(const std::vector<ExportPluginDescription>& exportPluginDescriptions,
                           const ExportData& exportData, Options& options)
        {
            auto it = std::find_if(exportPluginDescriptions.begin(), exportPluginDescriptions.end(),
                                   [&](const auto& plugin)
            ...
            auto optionalArgument =
                (argument.empty()) ? std::nullopt : std::make_optional(argument);
    ...
    const std::string ExportOptionParser::ExportTypeCoberturaValue = "cobertura";
    ...
    {
        exportTypes_.emplace(Tools::LocalToWString(ExportOptionParser::ExportTypeHtmlValue),
                             OptionsExportType::Html);
        exportTypes_.emplace(Tools::LocalToWString(ExportOptionParser::ExportTypeCoberturaValue),
                             OptionsExportType::Cobertura);
        exportTypes_.emplace(Tools::LocalToWString(ExportOptionParser::ExportTypeBinaryValue),
                             OptionsExportType::Binary);
    }

    //----------------------------------------------------------------------------
    void ExportOptionParser::ParseOption(const ProgramOptionsVariablesMap& variablesMap,
                                         Options&                          options)
    {
        auto exportTypeStrCollection =
            variablesMap.GetValue<std::vector<std::string>>(ExportOptionParser::ExportTypeOption);

        for (const auto& exportTypeStr : exportTypeStrCollection)
        {
            auto exportData   = ParseExportData(Tools::LocalToWString(exportTypeStr));
            auto optionExport = CreateExport(exportTypes_, exportData);

            if (optionExport)
                options.AddExport(std::move(*optionExport));
            else if (!TryAddExportPlugin(exportPluginDescriptions_, exportData, options))
            {
                throw Plugin::OptionsParserException(Tools::ToLocalString(exportData.exportType) +
    ...
    //----------------------------------------------------------------------------
    void ExportOptionParser::AddOption(boost::program_options::options_description& options)
    {
        options.add_options()(
    ...
    std::wstring ExportOptionParser::GetExportTypeText() const
    {
        ...
        for (const auto& description : exportPluginDescriptions_)
        {
            exportArgumentInfos.push_back(
                { description.GetPluginName(), description.GetParameterDescription() });
        }

        std::wstring exports;
        for (const auto& argumentInfos : exportArgumentInfos)
        {
            exports += L"   " + argumentInfos.first + L": " + argumentInfos.second + L'\n';
        }
