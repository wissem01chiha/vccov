...
namespace CppCoverage
{
    ...
    {
        options.add_options()(
            (ProgramOptions::VerboseOption + "," + ProgramOptions::VerboseShortOption).c_str(),
            "Verbose mode.")(
            (ProgramOptions::QuietOption + "," + ProgramOptions::QuietShortOption).c_str(),
            "Quiet mode.")(
            (ProgramOptions::HelpOption + "," + ProgramOptions::HelpShortOption).c_str(),
            "Show help message.")(ProgramOptions::ConfigFileOption.c_str(),
                                  po::value<std::string>(), "Filename of a configuration file.");
    }

    //---------------------------------------------------------------------
    VCOV_OPTIONEXPORT_DLL std::string GetUnifiedDiffHelp()
    {
        return std::string("Format: <unifiedDiffPath>") + OptionsParser::PathSeparator +
                                  ...
                                  const std::vector<std::unique_ptr<IOptionParser>>& optionParsers)
    {
        ...
        options.add_options()(ProgramOptions::SelectedModulesOption.c_str(),
                              po::value<T_Strings>()->default_value({ all }, all)->composing(),
                              "The pattern that module's paths should match. Can have multiple "
                              "occurrences.")(
            ProgramOptions::ExcludedModulesOption.c_str(), po::value<T_Strings>()->composing(),
            "The pattern that module's paths should NOT match. Can "
            "have multiple occurrences.")(
            ProgramOptions::SelectedSourcesOption.c_str(),
            po::value<T_Strings>()->default_value({ all }, all)->composing(),
            "The pattern that source's paths should match. Can have multiple "
            "occurrences.")(ProgramOptions::ExcludedSourcesOption.c_str(),
                            po::value<T_Strings>()->composing(),
                            "The pattern that source's paths should NOT match. Can "
                            "have multiple occurrences.")(
            ProgramOptions::InputCoverageValue.c_str(), po::value<T_Strings>()->composing(),
            ("A output path of " + ExportOptionParser::ExportTypeOption + "=" +
             ExportOptionParser::ExportTypeBinaryValue +
             ". This coverage data will be merged with the current one. Can have "
             "multiple occurrences.")
                .c_str())(ProgramOptions::WorkingDirectoryOption.c_str(), po::value<std::string>(),
                          "The program working directory.")(
            ProgramOptions::CoverChildrenOption.c_str(),
            "Enable code coverage for children processes.")(
            ProgramOptions::NoAggregateByFileOption.c_str(),
            "Do not aggregate coverage for same file path.")(
            ProgramOptions::StopOnAssertOption.c_str(),
            "Do not continue after DebugBreak() or assert().")(
            ProgramOptions::UnifiedDiffOption.c_str(), po::value<T_Strings>()->composing(),
            GetUnifiedDiffHelp().c_str())(ProgramOptions::ContinueAfterCppExceptionOption.c_str(),
                                          "Try to continue after throwing a C++ exception.")(
            ProgramOptions::OptimizedBuildOption.c_str(),
            "Enable heuristics to support optimized build. See documentation for "
            "restrictions.")(ProgramOptions::ExcludedLineRegexOption.c_str(),
                             po::value<T_Strings>()->composing(),
                             "Exclude all lines match the regular expression. "
                             "Regular expression must match the whole line.")(
            ProgramOptions::SubstitutePdbSourcePathOption.c_str(),
            po::value<T_Strings>()->composing(),
            "Substitute the starting path defined in the pdb by a local "
    ...
    {
        options.add_options()((ProgramOptions::PluginOption).c_str(), "Plugin mode.")(
    ...
    const std::string ProgramOptions::ContinueAfterCppExceptionOption =
    ...
    //-------------------------------------------------------------------------
    ProgramOptions::ProgramOptions(const std::vector<std::unique_ptr<IOptionParser>>& optionParsers)
        : visibleOptions_{ "Usage: [options] -- program_to_run optional_arguments" },
          configurationOptions_{ "Command line and configuration file" },
          hiddenOptions_{ "Hidden" }, genericOptions_{ "Command line only" }
    {
    ...
    //-------------------------------------------------------------------------
    void ProgramOptions::FillVariableMap(int argc, const char** argv,
    ...
    //-------------------------------------------------------------------------
    void ProgramOptions::FillVariableMap(std::istream&                          istr,
    ...
    //-------------------------------------------------------------------------
    std::wostream& operator<<(std::wostream& ostr, const ProgramOptions& programOptions)
    {
