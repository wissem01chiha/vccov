...
namespace CppCoverage
{
    namespace
    {
        ...
        void CheckPattern(const std::string& name, const std::string& value)
        {
            ...
            {
                throw Plugin::OptionsParserException("Error: Invalid value \""
                                                     "--" +
                                                     name + ' ' + value + "\". " +
        ...
        //---------------------------------------------------------------------
        cov::Patterns GetPatterns(const ProgramOptionsVariablesMap& variablesMap,
                                  const std::string& selected, const std::string& excluded)
        {
            cov::Patterns patterns{ false };

            auto selectedPatterns = variablesMap.GetValue<std::vector<std::string>>(selected);
            ...
            auto excludedPatterns =
                variablesMap.GetOptionalValue<std::vector<std::string>>(excluded);
            if (excludedPatterns)
            {
        ...
        //---------------------------------------------------------------------
        boost::optional<cov::StartInfo> GetStartInfo(const ProgramOptionsVariablesMap& variablesMap)
        {
            const auto* programToRun =
                variablesMap.GetOptionalValue<std::string>(ProgramOptions::ProgramToRunOption);
            ...
            const auto* arguments = variablesMap.GetOptionalValue<std::vector<std::string>>(
            ...
            const auto* workingDirectory =
                variablesMap.GetOptionalValue<std::string>(ProgramOptions::WorkingDirectoryOption);
        ...
        void ParseConfigFile(const ProgramOptions&       programOptions,
                             ProgramOptionsVariablesMap& variablesMap, const std::string& path)
        {
            std::ifstream ifs(path.c_str());

            if (!ifs)
                throw Plugin::OptionsParserException("Cannot open config file: " + path);

            programOptions.FillVariableMap(ifs, variablesMap.GetVariablesMap());
        }

        //---------------------------------------------------------------------
        void AddInputCoverages(const ProgramOptionsVariablesMap& variablesMap, Options& options)
        {
            auto inputCoveragePaths = variablesMap.GetOptionalValue<std::vector<std::string>>(
            ...
            if (inputCoveragePaths)
            {
                for (const auto& path : *inputCoveragePaths)
                {
                    ...
                    {
                        throw Plugin::OptionsParserException("Argument of " +
                                                             ProgramOptions::InputCoverageValue +
                                                             " <" + path + "> does not exist.");
                    }
        ...
        //----------------------------------------------------------------------------
        void AddUnifiedDiff(const ProgramOptionsVariablesMap& variablesMap, Options& options)
        {
            auto unifiedDiffCollection = variablesMap.GetOptionalValue<std::vector<std::string>>(
            ...
            if (unifiedDiffCollection)
            {
                for (const auto& unifiedDiff : *unifiedDiffCollection)
                {
                    ...
                    {
                        throw Plugin::OptionsParserException("Unified diff root folder " +
        ...
        //----------------------------------------------------------------------------
        void AddExcludedLineRegexes(const ProgramOptionsVariablesMap& variablesMap,
                                    Options&                          options)
        {
            auto excludedLineRegexes = variablesMap.GetOptionalValue<std::vector<std::string>>(
        ...
        SubstitutePdbSourcePath CreateSubstitutePdbSourcePath(const std::string& paths)
        {
            ...
            const auto error =
                "Invalid value for " + ProgramOptions::SubstitutePdbSourcePathOption + ". ";
            if (pos == std::string::npos)
            {
            ...
            {
                throw Plugin::OptionsParserException(error + "Path \"" + pdbPath +
        ...
        //---------------------------------------------------------------------
        void AddSubstitutePdbSourcePaths(const ProgramOptionsVariablesMap& variablesMap,
                                         Options&                          options)
        {
            auto substitutePdbSourcePaths = variablesMap.GetOptionalValue<std::vector<std::string>>(
            ...
            if (substitutePdbSourcePaths)
            {
                ...
                {
                    options.AddSubstitutePdbSourcePath(CreateSubstitutePdbSourcePath(paths));
                }
            }
        }
        //---------------------------------------------------------------------------
        void CheckArgumentsSize(int argc, const char** argv, Tools::WarningManager& warningManager)
        {
    ...
    //-------------------------------------------------------------------------
    OptionsParser::OptionsParser(std::shared_ptr<Tools::WarningManager>        warningManager,
    ...
    //-------------------------------------------------------------------------
    boost::optional<Options> OptionsParser::Parse(int argc, const char** argv,
    ...
    boost::optional<Options> OptionsParser::Parse(int argc, const char** argv) const
    {
        ...
        programOptions_->FillVariableMap(argc, argv, variablesMap.GetVariablesMap());
        const auto* configFile =
            variablesMap.GetOptionalValue<std::string>(ProgramOptions::ConfigFileOption);
        ...
        auto modulePatterns = GetPatterns(variablesMap, ProgramOptions::SelectedModulesOption,
                                          ProgramOptions::ExcludedModulesOption);
        auto sourcePatterns = GetPatterns(variablesMap, ProgramOptions::SelectedSourcesOption,
        ...
        bool isVerbose = variablesMap.IsOptionSelected(ProgramOptions::VerboseOption);
        bool isQuiet   = variablesMap.IsOptionSelected(ProgramOptions::QuietOption);

        if (isVerbose && isQuiet)
            throw Plugin::OptionsParserException("--" + ProgramOptions::VerboseOption + " and --" +
        ...
        if (variablesMap.IsOptionSelected(ProgramOptions::NoAggregateByFileOption))
            options.DisableAggregateByFileMode();
        if (variablesMap.IsOptionSelected(ProgramOptions::ContinueAfterCppExceptionOption))
        ...
        if (!options.GetStartInfo() && options.GetInputCoveragePaths().empty())
            throw Plugin::OptionsParserException("You must specify a program to execute or use --" +
                                                 ProgramOptions::InputCoverageValue);
    ...
    std::wstring OptionsParser::GetTooLongCommandLineMessage()
    {
        return L"You have a very long command line. It might be truncated "
               ...
               std::to_wstring(DosCommandLineMaxSize) + L". Please consider using --" +
               Tools::LocalToWString(ProgramOptions::ConfigFileOption) + L" instead.";
    }
} // namespace CppCoverage
