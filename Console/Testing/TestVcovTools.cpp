...
namespace OpenCppCoverageTest
{
    ...
    {
        coverageArguments.push_back({ cov::ExportOptionParser::ExportTypeOption,
    ...
    {
        return { cov::ExportOptionParser::ExportTypeOption,
    ...
    int RunCoverageFor(const std::vector<std::pair<std::string, std::string>>& coverageArguments,
                       const std::filesystem::path&                            programToRun,
                       const std::vector<std::wstring>& arguments, std::string* optionalOutput)
    {
        ...
        auto handle = Poco::Process::launch(openCppCoverage.string(), allCoverageArguments, ".",
                                            nullptr, pipe.get(), pipe.get());
        if (pipe)
        {
