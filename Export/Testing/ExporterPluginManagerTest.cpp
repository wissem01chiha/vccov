...
namespace ExporterTest
{
    ...
    {
        //---------------------------------------------------------------------
        class PluginLoaderMock : public Exporter::IPluginLoader<Plugin::IExportPlugin>
        {
          public:
            MOCK_CONST_METHOD2(TryLoadPlugin,
        ...
        {
          public:
            MOCK_METHOD2(Export, std::optional<std::filesystem::path>(
    ...
    class ExporterPluginManagerTest : public ::testing::Test
    {
      public:
        //---------------------------------------------------------------------
        ExporterPluginManagerTest() : pluginFolder_(TestHelper::TemporaryPathOption::CreateAsFolder)
        {
        ...
        CreateManager(std::unique_ptr<ExportPluginMock> exportPlugin)
        {
            ...
            EXPECT_CALL(pluginLoader, TryLoadPlugin(pluginPath_, _))
                .WillOnce(testing::Invoke(
                    [&](const auto& p, const auto&)
                    {
                        EXPECT_EQ(pluginPath_, p);
                        auto plugin =
                            std::make_unique<Exporter::LoadedPlugin<Plugin::IExportPlugin>>(
                                nullptr);
        ...
        //---------------------------------------------------------------------
        std::unique_ptr<ExportPluginMock>
        CreateExportPluginMock(int pluginVersion = Plugin::CurrentExportPluginVersion) const
        {
    ...
    TEST_F(ExporterPluginManagerTest, CreateExportPluginDescriptions)
    {
        ...
        auto pluginDescriptions = pluginManager->CreateExportPluginDescriptions();
        ...
        pluginManager.reset();
        ASSERT_THROW(pluginDescription.CheckArgument(argument), Exporter::ExporterException);
    }

    //-------------------------------------------------------------------------
    TEST_F(ExporterPluginManagerTest, InvalidVersion)
    {
        auto exportPlugin = CreateExportPluginMock(Plugin::CurrentExportPluginVersion);
        ASSERT_NO_THROW(CreateManager(std::move(exportPlugin)));

        exportPlugin = CreateExportPluginMock(Plugin::CurrentExportPluginVersion + 1);
        ASSERT_THROW(CreateManager(std::move(exportPlugin)), std::runtime_error);
    }
    ...
    TEST_F(ExporterPluginManagerTest, TryLoadPluginFailure)
    {
        ...
        TestHelper::AssertThrow<std::runtime_error>(
            ...
            [&](const auto& e) { EXPECT_THAT(e.what(), testing::HasSubstr(errorMessage)); });
    }
    ...
    TEST_F(ExporterPluginManagerTest, InvalidExport)
    {
        ...
        ASSERT_THROW(pluginManager->Export(pluginName_, coverageData, L""), std::runtime_error);

        TestHelper::AssertThrow<std::runtime_error>(
            [&]() { pluginManager->Export(pluginName_, coverageData, L""); },
            [&](const auto& e) { EXPECT_THAT(e.what(), testing::HasSubstr(errorMessage)); });

        ASSERT_THROW(pluginManager->Export(L"InvalidPluginName", coverageData, L""),
