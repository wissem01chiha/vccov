...
namespace ExporterTest
{
    namespace
    {
        ...
        //-----------------------------------------------------------------
        void FillSources(const SourceLines&               sourceLines,
                         ...
                         Plugin::FileCoverage&            fileCoverage)
        {
            ...
            for (const auto& line : sourceLines)
            {
                ...
                case CoverageType::Cover:
                    fileCoverage.AddLine(lineNumber, true);
                    break;
                case CoverageType::UnCover:
                    fileCoverage.AddLine(lineNumber, false);
                    break;
                case CoverageType::NotExecutable:
                    break;
                }
                ++lineNumber;
            }
        ...
        //-----------------------------------------------------------------
        std::vector<std::wstring> GetExportedLines(const SourceLines& sourceLines)
        {
    ...
    TEST(HtmlFileCoverageExporterTest, SeveralLines)
    {
        ...
        auto exportedLines              = GetExportedLines({ { lines.at(0), CoverageType::UnCover },
